...
#include "amf_sap.h"
#include "SubscriberdbServiceClient.h"

#define AMF_CAUSE_SUCCESS (1)
#define MAX_5G_AUTH_VECTORS 1

using magma5g::AsyncSubscriberdbServiceClient;
...
static int start_authentication_information_procedure(
  ...
  if (is_initial_req) {
    AsyncSubscriberdbServiceClient::getInstance().get_subs_auth_info(
        imsi_str, IMSI_LENGTH, ue_id);
  } else if (auts->data) {
    AsyncSubscriberdbServiceClient::getInstance().get_subs_auth_info_resync(
  ...
  switch (msg->m5gmm_cause.m5gmm_cause) {
    ...
    case AMF_CAUSE_SYNCH_FAILURE: {
      ...
      memcpy(
          ...
          msg->auth_failure_ie.authentication_failure_info, AUTS_LENGTH);
