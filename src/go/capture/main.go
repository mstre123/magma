...
func main() {
	...
	// Get path to intercept file name from middleware config.
	...
	interceptFileName := cfgr.Config().GetSctpdDownstreamMiddlewareConfig().GetInterceptOutputPath()

	makefile := "/home/vagrant/magma/lte/gateway/python/integ_tests/defs.mk"
	dir := "/home/vagrant/magma/lte/gateway/python/integ_tests"
	out := "/home/vagrant/magma/src/go/capture/resources/%s.golden"

	runAndCaptureTests(dir, out, interceptFileName, parsePrecommitTestsFromMakefile(makefile))
}
...
func runAndCaptureTests(dir string, outputPathFmt string, interceptFilePath string, tests []string) {
	// Clean incase file lingered from previous run.
	os.Remove(interceptFilePath)
	...
	for _, test := range tests {
		...
		// Read in intercept.
		readBytes, err := os.ReadFile(interceptFilePath)
		if err != nil {
			println("failed to read in intercept")
			panic(err)
		}

		// Delete intercept file.
		err = os.Remove(interceptFilePath)
		if err != nil {
			println("failed to delete intercept")
			panic(err)
		}
		...
		err = os.WriteFile(goldenfilename, readBytes, 0666)
		if err != nil {
